...
void runCurl() {
  ...
  while (p.available()>0) {
    ...
    if (c == '*'){             //astrick will be at the start of the text file
      for(int lth = 0; lth < 110; lth++){ 
        for (int ct = 0; ct < 32; ct++){ //length of data for one frame
          char e = p.read();
          bufferdata += e;
        }
        for (int ct = 0; ct < 1; ct++){ //don't keep last char bc it is a comma
          char d = p.read();
        }
...
//    }
    for (int adrs = 0; adrs <= 31; adrs++){  
      lightLED(adrs, 0); //turn all lights on
    }
    delay(200);
    for (int adrs = 0; adrs <= 31; adrs++){  
